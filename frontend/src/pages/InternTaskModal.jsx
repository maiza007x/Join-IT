...
const InternTaskModal = ({ visible, onHide, taskData = null, onSuccess }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            let response;
            if (isEditMode) {
                response = await api.put(`/tasks/edit-intern-task/${taskData.id}`, payload);
            } else {
                response = await api.post("/tasks/create-intern-task", payload);
            }
            ...
            if (response.data.success) {
                toast.current?.show({ 
                    severity: "success", 
                    summary: "สำเร็จ", 
                    detail: isEditMode ? "แก้ไขงานเรียบร้อยแล้ว" : "สร้างรายการงานเรียบร้อยแล้ว", 
                    life: 3000 
                });
                setTimeout(() => {
                    onSuccess();
    ...
    return (
        <Dialog
            header={isEditMode ? "แก้ไขงาน" : "สร้างงาน (นักศึกษา)"}
            visible={visible}
            onHide={onHide}
        ...
        >
            ...
            <form onSubmit={handleSubmit} className="p-4 md:p-6 bg-white">
                ...
                <div className="mt-8 flex justify-end gap-3">
                    ...
                    />
                    <Button
                        label={isEditMode ? "บันทึกการแก้ไข" : "ยืนยันสร้างงาน"}
                        type="submit"
                        loading={loading}
...
export default InternTaskModal;
